"""Deletion study of the chunk state kernel: start from the full epilogue
and strip pieces to find what separates it from the ~78us streaming floor.

  3 = full (stack load, s0 dots, rc/ru dots, Mt/C dots, o + u stores)
  2 = drop Mt/C loads+dots and the u store
  1 = also drop rc/ru loads+dots
  0 = also replace the stack load with a constant (= the old floor probe)
"""
import sys
...
def _state3(
    ...
    if LVL >= 1:
        ...
        stq = tl.full((TP, K), 0.01, tl.float32)
        stk = tl.full((TP, K), 0.02, tl.float32)
...
def main():
    ...
    for lvl in (0, 1, 2, 3):
        for BV, nw in [(64, 4), (64, 8), (128, 8)]:
            def launch():
                return _state3[(B * Hn, Vd // BV)](
                    t["v"], t["S"], t["old_u"], t["pnat"], t["buf_idx"],
                    stack, rc, mc, out, LVL=lvl,
                    B=B, Hn=Hn, T=T, HIST=HIST, K=K, Vd=Vd, BV=BV,
                    num_warps=nw)
            h = launch()
            ms = triton.testing.do_bench(launch, warmup=20, rep=100)
            print(f"lvl={lvl} BV={BV:3d} warps={nw}: {ms*1e3:7.1f} us"
