...
def parse_args():
    ...
    parser.add_argument(
        "--csv", type=Path, default=(Path(__file__).resolve().parents[2] / "results" / "legacy" / "bench_attention_layer.csv")
    )
    parser.add_argument(
        "--figure",
        type=Path,
    ...
    parser.add_argument(
        "--trace", type=Path, default=(Path(__file__).resolve().parents[2] / "results" / "legacy" / "kda_attention_trace.json")
    )
