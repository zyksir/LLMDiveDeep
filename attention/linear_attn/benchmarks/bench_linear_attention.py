...
def parse_args():
    ...
    parser.add_argument(
        "--csv", type=Path, default=(Path(__file__).resolve().parents[2] / "results" / "linear" / "bench_linear_attention.csv")
    )
    parser.add_argument(
        "--figure",
        type=Path,
    ...
    parser.add_argument(
        "--trace", type=Path, default=(Path(__file__).resolve().parents[2] / "results" / "linear" / "kda_decode_trace.json")
    )
