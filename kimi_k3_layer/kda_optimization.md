...
## 1. Decode (`bench_kda_kimi_k3.py`, graph replay, B=1..16)
...
swap test (debug/kda_decode_gemm_swap.py) DISPROVED the obvious next
...
## 2. Prefill (`bench_kda_prefill_layer.py`, eager, B=1, S=4k/8k/16k)
...
`debug/kda_prefill_gate_debug.py`, S=512, against the exact fp32
...
|---|---|---|---|
| decode attn_res kernel | measured | — | NEGATIVE in-layer: CuTeDSLGen kernel (attn_res_cutedsl.py) is correct and 5.0 us standalone (vs ~7 TRT) but +1.5-2 us SLOWER inside the layer and breaks graph capture at B=1 (PDL-chain interaction). Vendored, default off (B10_ATTNRES_KERNEL). |
...
```bash
# decode (single GPU)
CUDA_VISIBLE_DEVICES=4 python3 kimi_k3_layer/bench_kda_kimi_k3.py \
    --shards tp8 --batch-sizes 1 2 4 8 16
# prefill layer-level
