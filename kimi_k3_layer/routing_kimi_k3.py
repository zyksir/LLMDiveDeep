...
Honest accounting (bench_routing_kimi_k3.py, B200) - on clean fp32
contiguous logits the noaux kernel is nearly as good as ours (6.9 vs
...
baseline op cannot do:
  ...
  * read the layer's ACTUAL logits - a bf16 STRIDED slice of the
    merged input-GEMM output - at full speed. noaux accepts that input
    but slows to 8.2-10.4 us on it (B=1..256); ours runs 6.0-7.6 us.
  * store the consumer's exact format directly (fused_moe int32 ids +
    ...
    via `route_pack`), so no unpack/cast kernels follow. The noaux
    output always needs a values.float() cast kernel (~1.2 us).

Net at decode sizes: ~6.0 us vs ~9.3 us for noaux+cast, ~3.3 us per
MoE layer. Beyond ~1k tokens the cooperative noaux kernel amortizes
better and layers switch back (TRITON_ROUTE_MAX_TOKENS). The MoE bench
ablation (`bench_moe_kimi_k3.py --ablate`) measures this switch in the
full layer; if it stops paying there, default to noaux.
"""
...
# at 4k tokens, builtin -10% at 8k. See bench_routing_kimi_k3.py.
...
def route_for_fused_moe(
    ...
    the kernel - no unpack/cast kernels afterwards (the noaux baseline
